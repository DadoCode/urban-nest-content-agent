...
def _build_prompt(brand: dict, content_type: dict, chosen_format: str, property_record: dict | None) -> str:
    lines = [
...
def _template_fallback(brand: dict, content_type: dict, property_record: dict | None) -> dict:
    ...
    if property_record is not None:
        ...
        return {
            ...
            "hook": f"Your next stay in {p['area']} could look like this.",
            "visual_needed": (
    ...
    templates = {
        ...
        "shortlet_vs_hotel": {
            ...
            "hook": "Same trip, more space, better value.",
        ...
        "corporate_longstay": {
            ...
            "hook": "Relocating for work shouldn't mean living out of a suitcase.",
        ...
        "landlord_facing": {
            ...
            "hook": "Your property, fully managed, zero hassle.",
        ...
        "offers": {
            ...
            "hook": "A limited-time reason to book your next stay.",
        ...
        "brand_lifestyle": {
            ...
            "hook": "Home, wherever you're headed.",
    ...
    return templates[content_type["key"]]
...
def generate_post(
    ...
    chosen_format = rng.choice(content_type["typical_formats"])

    if os.environ.get("ANTHROPIC_API_KEY"):
        prompt = _build_prompt(brand, content_type, chosen_format, property_record)
        try:
            body = _call_claude(prompt)
        except Exception as exc:  # network/parsing issues -> fall back, don't crash the run
            body = _template_fallback(brand, content_type, property_record)
